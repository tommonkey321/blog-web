...
(function(){
    ...
    var dialog = {
         ...
         * @content 内容
         * */
        sysNotice:function(options){
            var type = options.type;
            var timeout = options.timeout || 1000*600;
            var content = options.content||"";
            var position = options.position || "";
            var title = options.title || "通知";
            var msgType = options.msgType || "[系统通知]"
            var iconClass = "";
            if(type==comm.dialog.type.warn){
                iconClass = "pui-notice-icon-warning";
            }else if(type==comm.dialog.type.error){
            ...
            var tmpClass="";
            if(position=="center"){
                iconClass = iconClass + " msg_pui-notice-center";
                tmpClass = "msg_";
            }
            ...
            var article = $("<article class='pui-notice pui-notice-icon "+iconClass+"' style='z-index:999'>" +
                ...
                "<header class='pui-notice-head'><strong>"+title+"</strong><small>"+msgType+"</small></header>" +
                "<div class='pui-notice-content "+tmpClass+"pui-notice-content-scrollable'><p style='font-size: 13px'>"+content+"</p></div>"+
                "</article>");
            var closeBtn =article.find(".pui-close").bind($.clickOrTouch(), function() {
                $(this).parent().css({minHeight: 0}).slideUp(2000, function() {
                    $(this).remove();
                });
            });
            if(timeout!=null && timeout>0){
